...
const CONTRACT_ADDRESS = "0x6fE26045438171AA2B27BEA8f07959a32C1BbbCd";
...
async function testContract() {
  try {
    ...
    if (functionExists) {
      ...
      try {
        const isMinted = await contract.isProjectMinted("cmh0tsafl00008zxq6fyxz56w");
        console.log("🔍 Project already minted:", isMinted);
        
        if (isMinted) {
          console.log("❌ Project is already minted - this is why gas estimation fails");
          return;
        }
      } catch (checkError) {
        console.log("⚠️ Could not check if project is minted:", checkError.message);
      }
